...
describe("Groq LLM Integration Tests", () => {
  ...
  const delayBetweenTests = async (ms: number = 21000) => {
  ...
  test(apiKey ? "Groq real: Juventus x Pafos (Total Gols + Francisco Conceição)" : "SKIP: Groq não configurado", async () => {
    ...
    const groqClient = new GroqLlmClient({ apiKey });
    ...
    jest.restoreAllMocks();
    const result = await semanticTicketLLM({ lines }, groqClient);

    console.log("\n=== APOSTAS EXTRAÍDAS (Groq Real) ===");
    result.apostasDetalhadas.forEach((a, i) => {
      console.log(
        `${i + 1}. tipo=${a.tipo}, jogador=${a.jogador}, ` +
        `estatistica=${a.estatistica}, condicao=${a.condicao}`
      );
    });

    // Validações
    const totalGols = result.apostasDetalhadas.find(
      (a) => a.estatistica === "Gols" && a.tipo === "match_prop"
    );
    expect(totalGols).toBeDefined();
    expect(totalGols?.condicao).toContain("2.5");

    const francisco = result.apostasDetalhadas.find(
      (a) => a.jogador?.includes("Francisco") && a.tipo === "player_prop"
    );
