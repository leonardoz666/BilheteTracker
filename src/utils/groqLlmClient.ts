    ...
    try {
      ...
      for (let i = 0; i < maxRetries; i++) {
        ...
        if ([429, 401, 403].includes(res.status) && this.getApiKey && i < maxRetries - 1) {
          if (this.onKeyFailure) this.onKeyFailure(this.apiKey, res.status);
          continue;
        }
...
- Se as linhas mencionarem estatísticas típicas de basquete
  ("ressaltos", "rebotes", "assistências", "pontos", "3PT" etc.),
  assuma que o esporte é "Basquete", a menos que haja evidência muito
  clara em contrário.
- Se as linhas mencionarem "gols", "escanteios", "cartões" ou termos
  como "placar", assuma que o esporte é "Futebol", a menos que outra
  informação seja mais forte.
- Use sempre os NOMES dos times/jogadores para inferir o esporte quando
  possível (por exemplo, equipes da NBA indicam basquete).
- Se houver conflito entre palavras genéricas e nomes claros de equipes/
  jogadores, priorize os nomes conhecidos e, em caso de dúvida, devolva
  "esporte": null em vez de arriscar um esporte errado.
