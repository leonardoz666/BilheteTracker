 ...
 */
export const NFL_CURRENT_TEAMS = [
  // AFC East
  "Buffalo Bills",
  "Miami Dolphins",
 ...
 */
export const NFL_ABBREVIATIONS: Record<string, string> = {
  // AFC East
  "BUF": "Buffalo Bills",
  "MIA": "Miami Dolphins",
  "NE": "New England Patriots",
 ...
 */
export const OTHER_FOOTBALL_LEAGUES = {
 ...
 */
export function normalizeNFLTeam(teamName: string): string | undefined {
  if (!teamName) return undefined;

  const normalized = teamName.trim().toLowerCase();

  // 1. Verificar em times atuais (case-insensitive)
  for (const team of NFL_CURRENT_TEAMS) {
  ...
  for (const [alias, official] of Object.entries(NFL_TEAM_ALIASES)) {
  ...
  for (const [abbrev, official] of Object.entries(NFL_ABBREVIATIONS)) {
 ...
 */
export function findOtherFootballTeam(teamName: string): { league: string; team: string } | undefined {
  if (!teamName) return undefined;

  const normalized = teamName.trim().toLowerCase();

  for (const [league, teams] of Object.entries(OTHER_FOOTBALL_LEAGUES)) {
