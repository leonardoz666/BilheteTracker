...
function detectPeriodo(line: string): PeriodoAposta {
...
function detectPeriodoFromContext(lines: string[], index: number): PeriodoAposta {
  return (
...
function getGlobalPeriodo(lines: string[]): PeriodoAposta {
  for (let i = 0; i < lines.length; i++) {
...
function isOrphanPeriod(line: string): boolean {
...
function mergeOrphanPeriods(lines: string[]): string[] {
  const result: string[] = [];
...
function cleanEstatistica(text: string): string {
  return text
...
function normalizeCondicao(condicao: string | null): string | null {
  if (!condicao) return null;
  
  const match = condicao.match(/(Mais de|Menos de)\s+(\d+(?:[.,]\d+)?)/i);
  if (!match) return condicao;
...
function normalizeEstatistica(text: string): string | null {
  const t = text.toLowerCase();

  // 🏀 Basketball / NBA
  if (/3\s*pontos/.test(t) || /3pt/.test(t)) return "Cestas de 3 Pontos";
  if (/cestas/.test(t)) return "Cestas";
  if (/rebotes/.test(t)) return "Rebotes";
  if (/assist/.test(t)) return "Assistências";
  if (/finaliza/.test(t)) return "Finalizações";
  if (/bloqueios?|tocos?/.test(t)) return "Bloqueios";

  // 🏈 NFL (Football Americano)
  if (/recepções?|recepçã|reception/.test(t)) return "Recepções";
  if (/touchdowns?|TD/.test(t)) return "Touchdowns";
  if (/yards?/.test(t)) return "Yards";
  if (/sacks?/.test(t)) return "Sacks";
  if (/intercepções?|intercept/.test(t)) return "Interceptações";
  if (/fumbles?/.test(t)) return "Fumbles";

  // ⚽ Football / Soccer
  if (/gols?/.test(t)) return "Gols";
  if (/chutes?/.test(t)) return "Chutes";
  if (/passes?/.test(t)) return "Passes";
  if (/defesas?/.test(t)) return "Defesas";
  if (/intercepta[çc][õo]es?/.test(t)) return "Interceptações";
  if (/roubos?/.test(t)) return "Roubos de Bola";

  return null;
}

// ✅ Valida se a string contém uma estatística válida (usa forma canônica)
function isValidEstatistica(text: string): boolean {
...
function stripEstatistica(line: string): string {
...
function canonicalPeriodoKey(periodo: string | null | undefined): string {
