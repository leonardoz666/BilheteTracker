...
exports.semanticTicketLLM = semanticTicketLLM;
const llmClient_1 = require("../utils/llmClient");
const extractMetadata_1 = require("./extractMetadata");
const extractRawBets_1 = require("./extractRawBets");
const semanticParserLLM_1 = require("./semanticParserLLM");
const formatterFinal_1 = require("./formatterFinal");
const enforcePlayerPropRule_1 = require("../utils/enforcePlayerPropRule");
...
function isValidAposta(aposta, allApostas, normalizedLines) {
    ...
    if (aposta.tipo === "winner") {
        ...
        const hasTrigger = normalizedLines.some(line => WINNER_TRIGGERS.some(trigger => line.toLowerCase().includes(trigger)));
        if (!hasTrigger) {
...
function shouldCreateFallbackAposta(candidate, existentes) {
    return !existentes.some((a) => a.tipo === "player_prop" &&
...
// Detector de período sem efeitos colaterais (não cria aposta)
function detectPeriodo(line) {
    const t = line.toLowerCase();
    if (/1[º°]\s*quarto/.test(t))
        return "1º Quarto";
    if (/2[º°]\s*quarto/.test(t))
        return "2º Quarto";
    if (/1[º°]\s*tempo|1st\s*half/.test(t))
        return "1º Tempo";
    if (/2[º°]\s*tempo|2nd\s*half/.test(t))
        return "2º Tempo";
    if (/intervalo|half\s*time|\bht\b/.test(t))
        return "1º Tempo";
    if (/jogo|ft|full\s*time/.test(t))
        return "Jogo";
    return null;
}
function detectPeriodoFromContext(lines, index) {
    return (detectPeriodo(lines[index] ?? "") ||
        detectPeriodo(lines[index - 1] ?? "") ||
        detectPeriodo(lines[index + 1] ?? "") ||
        null);
}
function getGlobalPeriodo(lines) {
    for (let i = 0; i < lines.length; i++) {
...
const IMMUTABLE_TYPES = ["winner", "match_prop", "team_prop"];
...
function detectCollectiveScope(text) {
    const t = text.toLowerCase();
    if (t.includes("ambas") || t.includes("both teams"))
        return "cada time";
    if (t.includes("cada time") || t.includes("cada equipe"))
        return "cada time";
    if (t.includes("dupla chance") || /(\b1x\b|\bx2\b|\b12\b)/i.test(t))
        return "dupla_chance";
    if (t.includes("ambas marcam") || t.includes("btts"))
        return "btts";
    return null;
}
// 🔎 Cluster Resultado do Jogo (Dupla Chance > Empate Anula > Winner)
function detectResultadoCluster(text) {
    const t = text.toLowerCase();
    if (/(\b1x\b|\bx2\b|\b12\b)/i.test(t) || t.includes("dupla chance"))
        return { tipo: "dupla_chance" };
    if (t.includes("empate anula") || t.includes("draw no bet"))
        return { tipo: "empate_anula" };
    if (t.includes("vencedor") || t.includes("resultado final"))
        return { tipo: "winner" };
    return null;
}
// 🔎 Detectores de AH e Totais
function detectAsianHandicap(text) {
    const t = text.toLowerCase();
...
function detectTotalGoals(text) {
    const t = text.toLowerCase();
    if (!(t.includes("gols") || t.includes("over") || t.includes("under")))
        return null;
    const over = /(?:mais de|over)\s*(\d+(?:[.,]\d+)?)/i.exec(text);
    const under = /(?:menos de|under)\s*(\d+(?:[.,]\d+)?)/i.exec(text);
    if (over)
        return { operador: "Mais de", linha: over[1].replace(",", ".") };
    if (under)
        return { operador: "Menos de", linha: under[1].replace(",", ".") };
    return null;
}
 ...
 */
function reconcileNFLPlayerProps(apostas) {
    ...
    const result = [];
    const merged = new Set(); // Marca apostas que foram CONSUMIDAS no merge
...
function isNFLSport(line, times = []) {
    // Verifica se tem time da NFL
    const hasNFLTeam = times.some(team => nfl_teams_1.NFL_CURRENT_TEAMS.includes(team));
    if (hasNFLTeam)
        return true;
    // Verifica se tem estatística exclusiva de NFL
    if (HAS_NFL_STAT_REGEX.test(line))
        return true;
    // Verifica se tem siglas de times de NFL na linha
    const abbrevMap = nfl_teams_1.NFL_ABBREVIATIONS;
    if (abbrevMap?.byCode) {
        for (const code of Object.keys(abbrevMap.byCode)) {
            if (new RegExp(`\\(${code}\\)`, 'i').test(line))
                return true;
        }
    }
    return false;
...
function isOrphanPeriod(line) {
    const trimmed = line.trim();
    // Padrões de período completo
    if (/^(1º|2º|3º|4º)\s*(Quarto|Tempo|Período)$/i.test(trimmed))
        return true;
    if (/^(HT|FT|Q[1-4]|1T|2T)$/i.test(trimmed))
        return true;
    if (/^Intervalo$/i.test(trimmed))
        return true;
    if (/^Jogo$/i.test(trimmed))
        return true;
    return false;
}
// 🔗 Mescla linhas de período órfãs com a próxima linha de aposta
// Exemplo: ["HT", "Nikola Jokic - Rebotes 1+"] → ["HT - Nikola Jokic - Rebotes 1+"]
function mergeOrphanPeriods(lines) {
    const result = [];
    for (let i = 0; i < lines.length; i++) {
...
function parseDeterministicPeriodPattern(line) {
    const trimmed = line.trim();
...
function buildPlayerPropFromMatch(periodoRaw, jogador, estatisticaCondicao) {
    // Normaliza período
    const periodoMap = {
        'ht': '1º Tempo',
        '1t': '1º Tempo',
    ...
    const periodo = periodoRaw
        ? (periodoMap[periodoRaw.toLowerCase()] || periodoRaw)
        : null;
...
function preprocessWithDeterministicParser(lines) {
    const parsedBets = [];
...
function isValidPlayerName(name) {
    ...
    for (const blocked of UI_BLACKLIST) {
        ...
        if (nameLower === blockedLower ||
            ...
            console.log(`🚫 Nome rejeitado (blacklist): "${name}"`);
            return false;
        }
    }
    return true;
...
function cleanEstatistica(text) {
    return text
...
function normalizeCondicao(condicao) {
    if (!condicao)
        return null;
    ...
    if (numeroBruto.includes(",") || numeroBruto.includes(".")) {
        return Number.isFinite(valor) ? `${operador} ${valor}` : condicao;
    }
...
// 🔧 Normaliza estatística para forma canônica (retorna null se não reconhecer)
function normalizeEstatistica(text) {
    const t = text.toLowerCase();
    // 🏀 Basketball / NBA
    if (/3\s*pontos/.test(t) || /3pt/.test(t))
        return "Cestas de 3 Pontos";
    if (/cestas/.test(t))
        return "Cestas";
    if (/rebotes/.test(t))
        return "Rebotes";
    ...
    if (/finaliza/.test(t))
        return "Finalizações";
    if (/bloqueios?|tocos?/.test(t))
        return "Bloqueios";
    // 🏈 NFL (Football Americano)
    if (/recepções?|recepçã|reception/.test(t))
        return "Recepções";
    if (/touchdowns?|TD/.test(t))
        return "Touchdowns";
    if (/yards?/.test(t))
        return "Yards";
    if (/sacks?/.test(t))
        return "Sacks";
    if (/intercepções?|intercept/.test(t))
...
function stripEstatistica(line) {
...
function buildDeterministicPlayerProp(line) {
    ...
    const tipo = jogadorNome ? "player_prop" : "match_prop";
    ...
    const timeNormalizado = timeAbrevFinal
        ? ((0, nba_teams_1.normalizeNBATeam)(timeAbrevFinal) || (0, football_clubs_1.normalizeFootballClub)(timeAbrevFinal) || null)
        : null;
    return {
        tipo: tipo,
        jogador: jogadorNome,
...
function existsClearPlayerPropLine(lines) {
    for (const line of lines) {
...
function isApostaValida(aposta) {
...
function enrichFromLine(aposta, line) {
...
function isDependentStatLine(line) {
    return /rebotes|assist[eê]ncias|pontos/i.test(line) && !extractPlayerName(line);
}
function getContextualPlayerName(lines, index) {
...
function enforcePlayerSafety(aposta, lines, index) {
...
function correctApostaTipo(aposta, lines, index) {
...
function getSemanticPriority(aposta) {
    if (aposta.tipo === "winner")
        return 1;
    // Mercados binários / estados da partida (Ambas Marcam, Dupla Chance, etc)
    if (aposta.tipo === "match_prop") {
        const isBinaryMarket = aposta.estatistica === "Ambas Marcam" ||
...
function applyDeterministicFallback(lines, apostas) {
    ...
    const apostasComPeriodoDefault = apostas.map((aposta) => {
        ...
        if (aposta.tipo === "player_prop" && !aposta.condicao) {
            const isBinaryAction = /chutar|chute|finaliza[çc]|marcar|assist|pass|defe|intercept|roub|bloq|toco/i.test(aposta.estatistica || "");
            if (isBinaryAction) {
    ...
    for (const line of lines) {
        ...
        if (btts) {
            const jaExiste = apostasComPeriodoDefault.some((a) => a.tipo === "match_prop" && a.estatistica === "Ambas Marcam");
            if (!jaExiste) {
...
function normalizeApostas(apostas) {
    return apostas.map((aposta) => {
        ...
        // Normaliza "Gols" para "Total de Gols" quando é match_prop
        if (aposta.tipo === "match_prop" &&
            estatistica?.toLowerCase() === "gols" &&
            aposta.condicao?.match(/mais de|menos de|over|under/i)) {
...
function forcePlayerPropIfPlayerPresent(bet) {
    if (bet.jogador && bet.tipo !== "player_prop") {
        return { ...bet, tipo: "player_prop" };
    }
    return bet;
}
function normalizePeriodoOnly(bet) {
...
function preventIllegalMatchProp(bet) {
...
function coercePlayerPropByRawLines(apostas, lines) {
...
function canonicalPeriodoKey(periodo) {
    const map = {
        ht: "1º Tempo",
        "1t": "1º Tempo",
        "1º tempo": "1º Tempo",
        ...
        ft: "Jogo",
        "tempo regulamentar": "Jogo",
        "full time": "Jogo",
    };
    const key = (periodo ?? "Jogo").toLowerCase();
    return map[key] ?? (periodo ?? "Jogo");
}
// 🔒 Mantém apenas uma linha de total de gols por período, normalizando operador/período e removendo conflitos OVER/UNDER
function enforceSingleTotalGols(apostas) {
    const others = [];
    ...
    for (const a of apostas) {
        ...
        const operador = operadorRaw.includes("mais") || operadorRaw.includes("over") ? "OVER" : "UNDER";
        const valor = parseFloat(condMatch[2].replace(",", "."));
        const periodoKey = canonicalPeriodoKey(a.periodo ?? "Jogo");
        const bucket = byPeriodo.get(periodoKey) ?? {};
        const existing = bucket[operador.toLowerCase()];
        if (!existing) {
            bucket[operador.toLowerCase()] = a;
    ...
    for (const [periodoKey, bucket] of byPeriodo.entries()) {
        ...
        if (bucket.over)
            resolved.push(bucket.over);
        if (bucket.under)
            resolved.push(bucket.under);
    }
    return [...others, ...resolved];
}
// 🔒 Normaliza semantics de "cada time"/"ambas equipes" baseado na linha fonte
function normalizeCadaTime(aposta, sourceLine) {
...
async function semanticTicketLLM(input, client = llmClient_1.defaultLlmClient) {
    console.log("USANDO TICKET LLM:", Boolean(client.callTicketParser));
    ...
    if (typeof client.callTicketParser === "function") {
        ...
        console.log("Apostas Extraídas do LLM:", JSON.stringify(full.apostasDetalhadas ?? [], null, 2));
        const hasMissingTeamAbbrev = (full.apostasDetalhadas ?? []).some((aposta) => aposta && !aposta.timeAbrev);
        ...
        const { metadata: localMetadata } = (0, extractMetadata_1.extractMetadata)(cleaned);
        ...
        const valorApostado = localMetadata.valorApostado ?? full.valorApostado ?? null;
        const odd = localMetadata.odd ?? full.odd ?? null;
        const retornoPotencial = localMetadata.retornoPotencial ?? full.retornoPotencial ?? null;
        ...
        let apostasBase = [
            ...
            ...(0, normalizeLabels_1.normalizeApostas)((0, enforcePlayerPropRule_1.enforceOnAll)(full.apostasDetalhadas ?? [])) // LLM
        ];
        ...
        if (esporte === "Futebol Americano") {
            ...
            apostasBase = apostasBase.filter((a) => {
                if (a.tipo === "match_prop") {
                    ...
                    if (isGols)
                        return false; // NFL não usa mercado de gols
                    if (hasPlayerProp)
                        return false; // Preferimos os player_props para NFL
                }
                return true;
            });
        ...
        let apostas = enforceSingleTotalGols(normalizeApostas(applyDeterministicFallback(cleaned, (0, normalizeLabels_1.normalizeApostas)((0, enforcePlayerPropRule_1.enforceOnAll)(baseBlindada)))));
        ...
        return (0, formatterFinal_1.formatBilhete)({
            esporte,
            torneio,
            evento,
            valorApostado,
    ...
    const { metadata, remainingLines: metadataRemainingLines } = (0, extractMetadata_1.extractMetadata)(cleaned);
    const globalPeriodoAll = getGlobalPeriodo(cleaned);
    const betLines = (0, extractRawBets_1.extractRawBets)(metadataRemainingLines);
    const semantic = await (0, semanticParserLLM_1.semanticParserLLM)(betLines, client);
    // 🎯 MESCLA: apostas determinísticas + apostas do LLM fallback
    let baseLocal = [
        ...deterministicBets, // ✅ Parser determinístico (100% confiável)
        ...(0, normalizeLabels_1.normalizeApostas)((0, enforcePlayerPropRule_1.enforceOnAll)(semantic.apostas ?? []))
    ];
    // 🔄 PIPELINE FINAL (ordem correta) — antes do fallback
    ...
    let apostas = enforceSingleTotalGols(normalizeApostas(applyDeterministicFallback(betLines, baseLocal)));
    if (globalPeriodoAll) {
        apostas = apostas.map((a) => {
            if (a.periodo)
                return a;
    ...
    const contratoViolado = apostas.some((a) => a.tipo === "match_prop" &&
        apostas.some((b) => b.tipo === "player_prop" &&
    ...
    const finalTicket = (0, formatterFinal_1.formatBilhete)(metadata, { apostas });
    return finalTicket;
}
