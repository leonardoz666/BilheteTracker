...
exports.NBA_EXTINCT_TEAMS = exports.NBA_TEAM_ALIASES = exports.NBA_CURRENT_TEAMS = void 0;
exports.normalizeNBATeam = normalizeNBATeam;
exports.isValidNBATeam = isValidNBATeam;
exports.getAllNBATeamNames = getAllNBATeamNames;
/**
 * Times atuais da NBA (30 franquias)
 */
exports.NBA_CURRENT_TEAMS = [
    // Conferência Leste
    "Atlanta Hawks",
    "Boston Celtics",
    "Brooklyn Nets",
 ...
 */
function normalizeNBATeam(teamName) {
    if (!teamName)
        return null;
    const trimmed = teamName.trim();
    // Verifica se já é um nome atual válido
    if (exports.NBA_CURRENT_TEAMS.includes(trimmed)) {
        return trimmed;
    }
    // Verifica se é um alias conhecido
    if (trimmed in exports.NBA_TEAM_ALIASES) {
        return exports.NBA_TEAM_ALIASES[trimmed];
    }
    // Busca case-insensitive
    const lowerInput = trimmed.toLowerCase();
    // Procura em times atuais
    const currentMatch = exports.NBA_CURRENT_TEAMS.find((team) => team.toLowerCase() === lowerInput);
    if (currentMatch)
        return currentMatch;
    // Procura em aliases
    const aliasKey = Object.keys(exports.NBA_TEAM_ALIASES).find((alias) => alias.toLowerCase() === lowerInput);
    if (aliasKey)
        return exports.NBA_TEAM_ALIASES[aliasKey];
    // Verifica se é um time extinto (não deve ser aceito)
    const isExtinct = exports.NBA_EXTINCT_TEAMS.some((extinct) => extinct.toLowerCase() === lowerInput);
    if (isExtinct) {
