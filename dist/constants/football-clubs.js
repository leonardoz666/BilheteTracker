...
exports.FOOTBALL_CLUB_ALIASES = exports.ALL_FOOTBALL_CLUBS = exports.FOOTBALL_CLUBS_BY_REGION = void 0;
exports.normalizeFootballClub = normalizeFootballClub;
exports.isValidFootballClub = isValidFootballClub;
exports.getFootballClubNames = getFootballClubNames;
exports.normalizeFootballEventTeams = normalizeFootballEventTeams;
/**
 * Times de futebol ativos com cobertura de apostas
 * Organizados por continente, país e liga
 */
exports.FOOTBALL_CLUBS_BY_REGION = {
    europe: {
        england: {
 ...
 */
exports.ALL_FOOTBALL_CLUBS = Object.values(exports.FOOTBALL_CLUBS_BY_REGION)
    .flatMap((continent) => Object.values(continent).flatMap((country) => Object.values(country).flat()))
    .filter((club) => typeof club === "string");
/**
 * Mapeamento de aliases para times de futebol
 * Inclui apelidos, abreviações e variações
 */
exports.FOOTBALL_CLUB_ALIASES = {
    // Brasil - Série A
 ...
 */
function normalizeFootballClub(clubName) {
    if (!clubName)
        return null;
    const trimmed = clubName.trim();
    // Busca exata (case-insensitive)
    const exactMatch = exports.ALL_FOOTBALL_CLUBS.find((club) => club.toLowerCase() === trimmed.toLowerCase());
    if (exactMatch)
        return exactMatch;
    // Busca em aliases
    const aliasMatch = exports.FOOTBALL_CLUB_ALIASES[trimmed];
    if (aliasMatch)
        return aliasMatch;
    // Busca case-insensitive em aliases
    const aliasKey = Object.keys(exports.FOOTBALL_CLUB_ALIASES).find((key) => key.toLowerCase() === trimmed.toLowerCase());
    if (aliasKey)
        return exports.FOOTBALL_CLUB_ALIASES[aliasKey];
    return null;
}
/**
 ...
 */
function isValidFootballClub(clubName) {
 ...
 */
function getFootballClubNames(canonicalName) {
    const canonical = normalizeFootballClub(canonicalName);
    if (!canonical)
        return [];
    const aliases = Object.entries(exports.FOOTBALL_CLUB_ALIASES)
 ...
 */
function normalizeFootballEventTeams(eventStr) {
    if (!eventStr)
        return null;
