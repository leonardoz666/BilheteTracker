...
exports.defaultLlmClient = exports.MockLlmClient = void 0;
...
function inferTipo(line) {
    ...
    if (lower.includes("jogador"))
        return "player_prop";
    if (lower.includes("cantos") || lower.includes("escanteios"))
        return "team_prop";
    return "match_prop";
}
function inferPeriodo(line) {
    const lower = line.toLowerCase();
    if (/(1º\s*quarto|1q)/.test(lower))
        return "1º Quarto";
    if (/(2º\s*quarto|2q)/.test(lower))
        return "2º Quarto";
    if (/(1º\s*tempo)/.test(lower))
        return "1º Tempo";
    if (/(2º\s*tempo)/.test(lower))
        return "2º Tempo";
    if (/(jogo todo|jogo inteiro|partida inteira)/.test(lower))
        return "Jogo";
    return null;
}
function inferConfianca(line) {
...
function extractPlayerAndCondition(raw) {
...
function extractStat(line) {
    const lower = line.toLowerCase();
    // Heurísticas rápidas para mercados globais sem prefixo explícito
    if (/handicap/.test(lower))
        return "Handicap Asiático";
    if (/(over|under|mais de|menos de)/.test(lower))
        return "Gols";
    ...
    const normalized = last.length > 1 ? last[0].toUpperCase() + last.slice(1).toLowerCase() : last;
    return normalized;
}
class MockLlmClient {
